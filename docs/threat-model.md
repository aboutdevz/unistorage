...
| **R**epudiation | Snapshot Engine (`pkg/enterprise/backup`) | Operator or external service disputes which files were backed up or modified during a scheduled retention run. | Every snapshot generates an immutable, timestamped `manifest.json` recording the canonical relative path, file size in bytes, modification time, and cryptographic SHA-256 hash for every file in the backup tree. | Snapshot manifest schema validation and retention pruner tests (`TestSnapshotRetention_Pruning`). |
...
| **D**enial of Service | Backup Scheduler (`pkg/enterprise/backup`) | Rapid or overlapping cron backup triggers launch concurrent sync jobs that exhaust disk I/O and corrupt state. | Snapshot backup engine enforces job-level mutual exclusion (`anti-double-run mutex`). Overlapping runs for the same target directory or policy are dropped or queued with structured warning logs. | Concurrency mutex unit tests (`TestSnapshot_AntiDoubleRunMutex`). |
